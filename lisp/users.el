...
(defun run-ask-ai ()
  ...
  (let ((background (buffer-string))
    ...
    (start-process "ai" buff "ask_ai.py" "-ae" "http://192.168.37.20:11434" "-m" "qwen2.5-coder:14b" "-fi" question)
    (process-send-string "*ollama*" background)
    (process-send-eof "*ollama*")
    (switch-to-buffer buff)))

(defun run-code-completion ()
  (interactive)
  (let ((buff (current-buffer))
	(background (buffer-string))
	(pt (point))
	(temp-file (make-temp-file "tmp-" nil)))
    (with-temp-buffer
      (insert background)
      (goto-char pt)
      (insert "{{CURSOR}}")
      (write-file temp-file)
      (call-process "code_completion.py" temp-file buff t "-l" "ERROR" "-ae" "http://192.168.37.20:11434" "-m" "qwen2.5-coder:14b"))
...
(global-set-key "\C-co" 'run-ask-ai)
(global-set-key "\C-c/" 'run-code-completion)
